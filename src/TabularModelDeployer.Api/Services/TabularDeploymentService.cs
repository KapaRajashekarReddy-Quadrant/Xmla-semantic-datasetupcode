...
public class TabularDeploymentService
{
    ...
    public string DeployModel(DeploymentRequest request)
    {
        ...
        foreach (var t in schema.Tables)
        {
            ...
            // 🔁 CHANGED: Physical tables now use Import instead of DirectQuery.
            // DirectQuery was live-querying the Lakehouse SQL analytics endpoint on
            // every single visual render, which under a burst of near-simultaneous
            // visual creations (see PowerBIReport.tsx bindVisualFields loop) was
            // intermittently exceeding query throughput and surfacing as
            // "QueryUserError ... capacity or license issue" — even though the
            // capacity itself was healthy. Import mode caches data in-memory, so
            // rendering visuals no longer depends on live SQL endpoint latency.
            // Tradeoff: data is only as fresh as the last refresh (see the
            // ProcessFull call after SaveChanges below, plus the existing
            // scheduled-refresh UI in PowerBIReport.tsx).
...
in
    Source";
        ...
        // 🔁 NEW: Import-mode partitions are metadata-only until processed.
        // Without this, tables/measures exist in the model but every visual
        // would query an empty cache — a different, silent failure mode.
        // ProcessFull loads (or reloads) data for every Import partition in
        // one pass; SaveChanges(commit) applies the processed data.
